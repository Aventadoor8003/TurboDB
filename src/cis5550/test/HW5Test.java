...
public class HW5Test extends GenericTest {
    ...
    void runTests(Set<String> tests) throws Exception {
        ...
        if (tests.contains("wget")) {
            try {
                startTest("wget", "When table or row doesn't exist", 10);

                String validTable = "pt-" + randomAlphaNum(4, 6);
                String rowKey = randomAlphaNum(4, 5);
                String columnKey = randomAlphaNum(4, 6);
                String dataValue = randomAlphaNum(4, 6);

                Socket s = openSocket(8001);
                PrintWriter out = new PrintWriter(s.getOutputStream());
                out.print("PUT /data/" + validTable + "/" + rowKey + "/" + columnKey + " HTTP/1.1\r\nContent-Length: " + dataValue.length() + "\r\nHost: localhost\r\n\r\n" + dataValue);
                out.flush();
                Response r = readAndCheckResponse(s, "response");
                if (r.statusCode != 200)
                    testFailed("During setup, expected 200 OK for PUT with valid table name with 'pt-' prefix but got: " + r.statusCode + ". Body:\n" + dump(r.body));
                s.close();

                s = openSocket(8001);
                out = new PrintWriter(s.getOutputStream());
                out.print("GET /data/" + validTable + "/" + rowKey + " HTTP/1.1\r\nHost: localhost\r\n\r\n");
                out.flush();
                r = readAndCheckResponse(s, "response");
                if (r.statusCode != 200 || !r.body().contains(dataValue))
                    testFailed("Expected 200 OK and body containing: " + dataValue + " for valid table name with 'pt-' prefix, but got: " + r.statusCode + ". Body:\n" + dump(r.body));
                s.close();
                ...
                s = openSocket(8001);
                out = new PrintWriter(s.getOutputStream());
                out.print("GET /data/" + invalidTable + "/" + rowKey + " HTTP/1.1\r\nHost: localhost\r\n\r\n");
                out.flush();
                r = readAndCheckResponse(s, "response");
                if (r.statusCode != 404)
                    testFailed("Expected 404 Not Found for invalid table name without 'pt-' prefix, but got: " + r.statusCode + ". Body:\n" + dump(r.body));
                s.close();

                String nonExistingRowKey = "nonExistingRow";

                s = openSocket(8001);
                out = new PrintWriter(s.getOutputStream());
                out.print("GET /data/" + validTable + "/" + nonExistingRowKey + " HTTP/1.1\r\nHost: localhost\r\n\r\n");
                out.flush();
                r = readAndCheckResponse(s, "response");
                if (r.statusCode != 404)
                    testFailed("Expected 404 Not Found for non-existing row key with valid table name, but got: " + r.statusCode + ". Body:\n" + dump(r.body));
                s.close();

                testSucceeded();
            } catch (Exception e) {
                testFailed("An exception occurred during the get-data-route test: " + e, false);
                e.printStackTrace();
            }
        }

        if (tests.contains("wget-t")) {
            try {
                startTest("wget-t", "When table doesn't exist", 10);
                Socket s = openSocket(8001);
                PrintWriter out = new PrintWriter(s.getOutputStream());
                String validTable = "pt-" + randomAlphaNum(4, 6);
                String rowKey = randomAlphaNum(4, 5);
                ...
                s = openSocket(8001);
                out = new PrintWriter(s.getOutputStream());
                out.print("GET /data/" + validTable  + " HTTP/1.1\r\nHost: localhost\r\n\r\n");
                out.flush();
                r = readAndCheckResponse(s, "response");
                if (r.statusCode != 200 || !r.body().contains(dataValue) || !r.body().contains(columnKey) || !r.body().contains(rowKey))
                    testFailed("Expected 200 OK and body containing: " + dataValue + " for valid table name with 'pt-' prefix, but got: " + r.statusCode + ". Body:\n" + dump(r.body));
                s.close();
                ...
                s = openSocket(8001);
                out = new PrintWriter(s.getOutputStream());
                out.print("GET /data/" + invalidTable + " HTTP/1.1\r\nHost: localhost\r\n\r\n");
                out.flush();
                r = readAndCheckResponse(s, "response");
                if (r.statusCode != 404)
                    testFailed("Expected 404 Not Found for invalid table name without 'pt-' prefix, but got: " + r.statusCode + ". Body:\n" + dump(r.body));
                ...
                testFailed("An exception occurred during the get-tables-nonexist test: " + e, false);
                e.printStackTrace();
            }
        }

        if (tests.contains("a-rename")) {
            try {
                startTest("a-rename", "Augmented rename test", 5);
                Socket s = openSocket(8001);
                PrintWriter out = new PrintWriter(s.getOutputStream());
                String thetable = randomAlphaNum(4, 6);
                String therow = randomAlphaNum(4, 5);
                ...
                //rename table not exist
                s = openSocket(8001);
                out = new PrintWriter(s.getOutputStream());
                out.print("PUT /rename/" + "notexist" + " HTTP/1.1\r\nHost: localhost\r\nContent-Length: " + thetable.length() + "\r\n\r\n" + thetable);
                out.flush();
                r = readAndCheckResponse(s, "response");
                if (r.statusCode != 404)
                    testFailed("The server returned a " + r.statusCode + " response to our PUT /rename/" + "notexist" + ", but we were expecting a 404 Not Found. Here is what was in the body:\n\n" + dump(r.body));
                s.close();
                ...
                s = openSocket(8001);
                out = new PrintWriter(s.getOutputStream());
                out.print("PUT /rename/" + newtable + " HTTP/1.1\r\nHost: localhost\r\nContent-Length: " + newtable.length() + "\r\n\r\n" + newtable);
                out.flush();
                r = readAndCheckResponse(s, "response");
                if (r.statusCode != 409)
                    testFailed("The server returned a " + r.statusCode + " response to our PUT /rename/" + newtable + ", but we were expecting a 409 Conflict. Here is what was in the body:\n\n" + dump(r.body));
                s.close();
    ...
    public static void main(String args[]) throws Exception {
        ...
        if ((args.length == 0) || args[0].equals("all") || args[0].equals("auto")) {
            ...
            tests.add("pages");
            tests.add("persist");
            //add test: read existing files from disk

            //a-test: augmented test
            //w-test: test contains wrong usage
            tests.add("wget");
            tests.add("wget-t");
            tests.add("a-rename");
        }
