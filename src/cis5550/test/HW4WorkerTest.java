...
public class HW4WorkerTest extends GenericTest {
    ...
    void runTests(Set<String> tests) throws Exception {
        ...
        if (tests.contains("getempty")) try {
            startTest("getempty", "Try to get non-exist values", 10);
            Socket s = openSocket(8001);
            PrintWriter out = new PrintWriter(s.getOutputStream());
            String cell = "/data/" + randomAlphaNum(5, 8) + "/" + randomAlphaNum(5, 8) + "/" + randomAlphaNum(5, 8);
            String data = randomAlphaNum(10, 20);
            ...
            // Now try to get a non-existent column from an existing row in an existing table
            s = openSocket(8001);
            out = new PrintWriter(s.getOutputStream());
            String nonExistentCell = cell.substring(0, cell.lastIndexOf("/")) + "/" + randomAlphaNum(5, 8);
            req = "GET " + nonExistentCell;
            out.print(req + " HTTP/1.1\r\nHost: localhost\r\n\r\n");
            out.flush();
            r = readAndCheckResponse(s, "response");
            if (r.statusCode != 404)
            ...
            // Now try to get a non-existent row from an existing table
            s = openSocket(8001);
            out = new PrintWriter(s.getOutputStream());
            String nonExistentRow = "/data/" + cell.split("/")[2] + "/" + randomAlphaNum(5, 8) + "/" + randomAlphaNum(5, 8);
            req = "GET " + nonExistentRow;
            out.print(req + " HTTP/1.1\r\nHost: localhost\r\n\r\n");
            out.flush();
            r = readAndCheckResponse(s, "response");
            if (r.statusCode != 404)
            ...
            // Now try to get a non-existent table
            s = openSocket(8001);
            out = new PrintWriter(s.getOutputStream());
            String nonExistentTable = "/data/" + randomAlphaNum(5, 8) + "/" + randomAlphaNum(5, 8) + "/" + randomAlphaNum(5, 8);
            req = "GET " + nonExistentTable;
            out.print(req + " HTTP/1.1\r\nHost: localhost\r\n\r\n");
            out.flush();
            r = readAndCheckResponse(s, "response");
            if (r.statusCode != 404)
        ...
        if (tests.contains("invget")) try {
            startTest("invget", "When one of table, row, column is non-existent", 5);

            // First, we will PUT some valid data
            Socket s = openSocket(8001);
            PrintWriter out = new PrintWriter(s.getOutputStream());
            String table = randomAlphaNum(5, 8);
            String row = randomAlphaNum(5, 8);
            String col = randomAlphaNum(5, 8);
            String dataPath = "/data/" + table + "/" + row + "/" + col;
            String data = randomAlphaNum(10, 20);
            String req = "PUT " + dataPath + " HTTP/1.1\r\nHost: localhost\r\nContent-Length: " + data.length() + "\r\n\r\n" + data;
            out.print(req);
            out.flush();
            Response r = readAndCheckResponse(s, "response");
            if (r.statusCode != 200 || !r.body().equals("OK")) {
                testFailed("Failed to PUT data. Expected 200 OK but got " + r.statusCode + ". Body: " + dump(r.body));
            }
            s.close();

            // Attempt to GET from a non-existent table
            s = openSocket(8001);
            out = new PrintWriter(s.getOutputStream());
            req = "GET /data/nonExistentTable/" + row + "/" + col + " HTTP/1.1\r\nHost: localhost\r\n\r\n";
            out.print(req);
            out.flush();
            r = readAndCheckResponse(s, "response");
            if (r.statusCode != 404) {
                testFailed("Expected 404 for non-existent table but got " + r.statusCode + ". Body: " + dump(r.body));
            }
            s.close();

            // Attempt to GET from a non-existent row
            s = openSocket(8001);
            out = new PrintWriter(s.getOutputStream());
            req = "GET /data/" + table + "/nonExistentRow/" + col + " HTTP/1.1\r\nHost: localhost\r\n\r\n";
            out.print(req);
            out.flush();
            r = readAndCheckResponse(s, "response");
            if (r.statusCode != 404) {
                testFailed("Expected 404 for non-existent row but got " + r.statusCode + ". Body: " + dump(r.body));
            }
            s.close();

            // Attempt to GET from a non-existent column
            s = openSocket(8001);
            out = new PrintWriter(s.getOutputStream());
            req = "GET /data/" + table + "/" + row + "/nonExistentColumn HTTP/1.1\r\nHost: localhost\r\n\r\n";
            out.print(req);
            out.flush();
            r = readAndCheckResponse(s, "response");
            if (r.statusCode != 404) {
                testFailed("Expected 404 for non-existent column but got " + r.statusCode + ". Body: " + dump(r.body));
            }
            s.close();
        ...
        if (tests.contains("condput")) try {
            startTest("condput", "Conditional PUT Test", 5);
            Socket s = openSocket(8001);
            PrintWriter out = new PrintWriter(s.getOutputStream());
            ...
            String data1 = randomAlphaNum(5, 8);
            String data2 = randomAlphaNum(5, 8);
            String data3 = randomAlphaNum(5, 8);

            // Initial PUT request
            String req1 = "PUT /data/" + table + "/" + row + "/" + column + " HTTP/1.1\r\nHost: localhost\r\nContent-Length: " + data1.length() + "\r\n\r\n" + data1;
            out.print(req1);
            out.flush();
            Response r = readAndCheckResponse(s, "response");
            if (r.statusCode != 200)
                testFailed("The server returned a " + r.statusCode + " response to our PUT request, but we were expecting a 200 OK.");

            // Attempt a conditional PUT with wrong expected value
            String req2 = "PUT /data/" + table + "/" + row + "/" + column + "?ifcolumn=" + column + "&equals=" + data2 + " HTTP/1.1\r\nHost: localhost\r\nContent-Length: " + data2.length() + "\r\n\r\n" + data2;
            out.print(req2);
            out.flush();
            r = readAndCheckResponse(s, "response");
            if (!r.body().equals("FAIL"))
                testFailed("The server returned a " + new String(r.response) + " response to our conditional PUT request, but we were expecting a FAIL response. Here is what was in the response:\n\n" + dump(r.response) + "\nMaybe you didn't return the correct response when the conditional PUT failed?");

            // Attempt a conditional PUT with correct expected value
            String req3 = "PUT /data/" + table + "/" + row + "/" + column + "?ifcolumn=" + column + "&equals=" + data1 + " HTTP/1.1\r\nHost: localhost\r\nContent-Length: " + data3.length() + "\r\n\r\n" + data3;
            out.print(req3);
            out.flush();
            r = readAndCheckResponse(s, "response");
            if (r.statusCode != 200)
                testFailed("The server returned a " + r.statusCode + " response to our conditional PUT request, but we were expecting a 200 OK.");

            // Validate the new data
            String req4 = "GET /data/" + table + "/" + row + "/" + column + " HTTP/1.1\r\nHost: localhost\r\n\r\n";
            out.print(req4);
            out.flush();
            r = readAndCheckResponse(s, "response");
            if (r.statusCode != 200)
                testFailed("The server returned a " + r.statusCode + " response to our GET request, but we were expecting a 200 OK.");
            if (!r.body().equals(data3))
                testFailed("Conditional PUT test failed. Expected data: " + data3 + " but got: " + r.body());
        ...
        if (tests.contains("version")) try {
            startTest("version", "Test versioning", 5);
            ...
            Response r = readAndCheckResponse(s, "response");
            if (r.statusCode != 200 || !r.body().equals("OK"))
                testFailed("PUT request failed");
            if(r.headers.get("version") == null)
                testFailed("PUT request did not return a version header " + new String(r.response));
            if(!"1".equals(r.headers.get("version")))
                testFailed("PUT request did not return the expected version, expected: 1 but got: " + r.headers.get("version"));
            s.close();
            ...
            r = readAndCheckResponse(s, "response");
            if (!r.body().equals(data1))
                testFailed("GET request did not return the expected data, expected: " + data1 + " but got: " + r.body());
            String version = r.headers.get("version");
            if(version == null)
                testFailed("GET request did not return a version header");
            if(!"1".equals(version))
                testFailed("GET request did not return the expected version, expected: 1 but got: " +version);
            s.close();
            ...
            if (r.statusCode != 200 || !r.body().equals("OK"))
                testFailed("PUT request failed");
            if(r.headers.get("version") == null)
                testFailed("PUT request did not return a version header");
            if(!"2".equals(r.headers.get("version")))
                testFailed("PUT request did not return the expected version, expected: 2 but got: " + r.headers.get("version"));
            s.close();
            ...
            r = readAndCheckResponse(s, "response");
            if (!r.body().equals(data2))
                testFailed("GET request did not return the expected data, expected: " + data2 + " but got: " + r.body());
            if(r.headers.get("version") == null)
                testFailed("GET request did not return a version header");
            if(!"2".equals(r.headers.get("version")))
                testFailed("GET request did not return the expected version, expected: 2 but got: " + r.headers.get("version"));
            s.close();

            s = openSocket(8001);
            out = new PrintWriter(s.getOutputStream());
            req = "GET " + cell + "?version=1";
            out.print(req + " HTTP/1.1\r\nHost: localhost\r\n\r\n");
            out.flush();
            r = readAndCheckResponse(s, "response");
            if (!r.body().equals(data1))
                testFailed("GET request with version did not return the expected data, expected: " + data1 + " but got: " + r.body());
            s.close();

            s = openSocket(8001);
            out = new PrintWriter(s.getOutputStream());
            req = "GET " + cell + "?version=3";
            out.print(req + " HTTP/1.1\r\nHost: localhost\r\n\r\n");
            out.flush();
            r = readAndCheckResponse(s, "response");
            if (r.statusCode != 404)
                testFailed("Expected 404 status code for non-existent version but got " + r.statusCode);
            s.close();
        ...
        if (tests.contains("invpar1")) try {
            startTest("invpar1", "Check behavior for non-existent data", 5);
        ...
        if (tests.contains("mispar")) try {
            startTest("mispar", "Check behavior for missing parameters", 5);
            ...
            s = openSocket(8001);
            out = new PrintWriter(s.getOutputStream());
            req = "PUT /data/" + table + "/" + row + "/" + col + " HTTP/1.1\r\nHost: localhost\r\nContent-Length: 5\r\n\r\nhello";
            out.print(req);
            out.flush();
            r = readAndCheckResponse(s, "response");
            if (r.statusCode != 200 || !r.body().equals("OK")) {
            ...
            // Attempt to GET missing table (t)
            s = openSocket(8001);
            out = new PrintWriter(s.getOutputStream());
            req = "GET /data/" + "/" + row + "/" + col + " " + "HTTP/1.1\r\nHost: localhost\r\n\r\n";
            out.print(req);
            out.flush();
            r = readAndCheckResponse(s, "response");
            if (r.statusCode != 404) {
                testFailed("Expected 404 for missing table parameter but got " + r.statusCode + ". Body: " + dump(r.body));
            }
            s.close();

            // Attempt to GET missing row (r)
            s = openSocket(8001);
            out = new PrintWriter(s.getOutputStream());
            req = "GET /data/" + table + "/" +  "" + "/" + col + " " + "HTTP/1.1\r\nHost: localhost\r\n\r\n";
            out.print(req);
            out.flush();
            r = readAndCheckResponse(s, "response");
            if (r.statusCode != 404) {
                testFailed("Expected 404 for missing row parameter but got " + r.statusCode + ". Body: " + dump(r.body));
            }
            s.close();

            // Attempt to GET missing column (c)
            s = openSocket(8001);
            out = new PrintWriter(s.getOutputStream());
            req = req = "GET /data/" + table + "/" + "" + "/" + col + " " + "HTTP/1.1\r\nHost: localhost\r\n\r\n";
            out.print(req);
            out.flush();
            r = readAndCheckResponse(s, "response");
            if (r.statusCode != 404) {
                testFailed("Expected 404 for missing column parameter but got " + r.statusCode + ". Body: " + dump(r.body));
            }
            s.close();
