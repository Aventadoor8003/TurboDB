...
public class HW4CoordinatorTest extends GenericTest {
    ...
    void runTests(Set<String> tests) throws Exception {
        ...
        if(tests.contains("pinginv")) try {
            setTimeoutMillis(25000);
            startTest("pinginv", "invalid ping tests", 5);
            Thread.sleep(20000);
            Socket s = openSocket(8000);
            PrintWriter out = new PrintWriter(s.getOutputStream());
            ...
            Response r = readAndCheckResponse(s, "response");
            if (r.statusCode != 400)
                testFailed("The server returned a " + r.statusCode + " response, but we were expecting a 400 Bad Request. Here is what was in the body:\n\n" + dump(r.body));
            s.close();

            s = openSocket(8000);
            out = new PrintWriter(s.getOutputStream());
            out.print("GET /ping?id=abcde&port=1234&extra=123 HTTP/1.1\r\nHost: localhost\r\n\r\n");
            out.flush();
            r = readAndCheckResponse(s, "response");
            if (r.statusCode != 200)
                testFailed("The server returned a " + r.statusCode + " response, but we were expecting a 200. Here is what was in the body:\n\n" + dump(r.body));
            s.close();
            ...
            s = openSocket(8000);
            out = new PrintWriter(s.getOutputStream());
            out.print("GET /ping?id=abcde&port=xxx&extra=123 HTTP/1.1\r\nHost: localhost\r\n\r\n");
            out.flush();
            r = readAndCheckResponse(s, "response");
            if (r.statusCode != 400)
                testFailed("The server returned a " + r.statusCode + " response, but we were expecting a 400 Bad Request. Here is what was in the body:\n\n" + dump(r.body));
            ...
            s = openSocket(8000);
            out = new PrintWriter(s.getOutputStream());
            out.print("GET /ping?id=abcde&port=xxx HTTP/1.1\r\nHost: localhost\r\n\r\n");
            out.flush();
            r = readAndCheckResponse(s, "response");
            if (r.statusCode != 400)
                testFailed("The server returned a " + r.statusCode + " response, but we were expecting a 400 Bad Request. Here is what was in the body:\n\n" + dump(r.body));
            ...
            s = openSocket(8000);
            out = new PrintWriter(s.getOutputStream());
            out.print("GET /nonexist HTTP/1.1\r\nHost: localhost\r\n\r\n");
            out.flush();
            r = readAndCheckResponse(s, "response");
            if (r.statusCode != 404)
                testFailed("The server returned a " + r.statusCode + " response, but we were expecting a 400 Bad Request. Here is what was in the body:\n\n" + dump(r.body));
    ...
    public static void main(String args[]) throws Exception {
        ...
        for (String arg : args)
            if (!arg.equals("all") && !arg.equals("auto") && !arg.equals("setup") && !arg.equals("cleanup"))
                tests.add(arg);
